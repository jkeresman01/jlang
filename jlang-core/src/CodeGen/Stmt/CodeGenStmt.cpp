...
void CodeGenerator::VisitIfStatement(IfStatement &node) {
  ...
  if (conditionValue->getType()->isIntegerTy(32)) {
    conditionValue = m_IRBuilder.CreateICmpNE(conditionValue,
                                              llvm::ConstantInt::get(conditionValue->getType(), 0), "ifcond");
  }
...
void CodeGenerator::VisitForStatement(ForStatement &node) {
  ...
  if (node.condition) {
    ...
    if (condValue->getType()->isIntegerTy(32)) {
      condValue =
          m_IRBuilder.CreateICmpNE(condValue, llvm::ConstantInt::get(condValue->getType(), 0), "forcond");
    }
...
void CodeGenerator::VisitForEachStatement(ForEachStatement &node) {
  ...
  if (isArray) {
    ...
    llvm::Value *sizePtr = m_IRBuilder.CreateStructGEP(vecInfo->llvmType, vecAlloca, 1, "foreach_size_ptr");
