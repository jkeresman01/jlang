...
void CodeGenerator::VisitCallExpr(CallExpr &node) {
  ...
  if (node.callee == "std::Vector" && !node.typeArguments.empty()) {
    ...
    m_IRBuilder.CreateStore(llvm::ConstantInt::get(llvm::Type::getInt64Ty(m_Context), initialCap), capField);
            ...
            if (ifaceInfo) {
              ...
              if (!argStructName.empty()) {
                ...
                if (siInfo && siInfo->vtableGlobal) {
                  ...
                  llvm::Value *dataPtr = m_IRBuilder.CreateBitCast(m_LastValue, i8Ptr, "arg_data");

                  llvm::AllocaInst *fatPtr =
                      m_IRBuilder.CreateAlloca(ifaceInfo->fatPtrType, nullptr, "arg_fat");

                  llvm::Value *dataField =
                      m_IRBuilder.CreateStructGEP(ifaceInfo->fatPtrType, fatPtr, 0, "arg_fat_data");
                  m_IRBuilder.CreateStore(dataPtr, dataField);

                  llvm::Value *vtableField =
                      m_IRBuilder.CreateStructGEP(ifaceInfo->fatPtrType, fatPtr, 1, "arg_fat_vtable");
                  m_IRBuilder.CreateStore(siInfo->vtableGlobal, vtableField);

                  // Load the fat pointer struct to pass by value
                  m_LastValue = m_IRBuilder.CreateLoad(ifaceInfo->fatPtrType, fatPtr, "arg_fat_val");
                }
              }
            }
          }
...
void CodeGenerator::VisitLiteralExpr(LiteralExpr &node) {
  if (node.value == "NULL" || node.value == "null" || node.value == "nullptr") {
    ...
    m_LastValue = llvm::ConstantInt::get(llvm::Type::getInt8Ty(m_Context), static_cast<uint8_t>(charValue));
