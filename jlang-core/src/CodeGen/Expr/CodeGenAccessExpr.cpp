...
void CodeGenerator::VisitMemberAccessExpr(MemberAccessExpr &node) {
  ...
  if (fieldIt == structInfo.fields.end()) {
    JLANG_ERROR(STR("Unknown field '%s' in struct '%s'", node.memberName.c_str(), structTypeName.c_str()));
    return;
  }
  ...
  llvm::Value *fieldPtr =
      m_IRBuilder.CreateStructGEP(structInfo.llvmType, objectPtr, fieldInfo.index, node.memberName + "_ptr");
...
void CodeGenerator::VisitMethodCallExpr(MethodCallExpr &node) {
  ...
  if (vecInfo) {
    ...
    if (node.methodName == "push") {
      ...
      llvm::Value *newSize = m_IRBuilder.CreateSub(size, llvm::ConstantInt::get(i64Type, 1), "new_size");
      ...
      llvm::Value *elemSizeVal = llvm::ConstantInt::get(i64Type, elemSize);
      llvm::Value *newData =
          m_IRBuilder.CreateCall(reserveFunc, {dataPtrVoid, capFieldPtr, elemSizeVal, newCap}, "new_data");
  ...
  if (ifaceInfo) {
    ...
    if (!methodInfo) {
      JLANG_ERROR(STR("Method '%s' not found in interface '%s'", node.methodName.c_str(), typeName.c_str()));
      return;
    }

    // Load data pointer (index 0)
    llvm::Value *dataField = m_IRBuilder.CreateStructGEP(ifaceInfo->fatPtrType, fatPtrAlloca, 0, "data_gep");
    ...
    llvm::Value *funcSlotPtr =
        m_IRBuilder.CreateStructGEP(ifaceInfo->vtableType, vtablePtr, methodInfo->vtableIndex, "func_slot");
  ...
  if (!func) {
    JLANG_ERROR(STR("Method '%s' not found (looked for '%s')", node.methodName.c_str(), mangledName.c_str()));
    return;
  }
