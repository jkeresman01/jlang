  ...
  if (!varName.empty()) {
    ...
    if (varInfo->type.isVector()) {
      ...
      if (vecInfo) {
        ...
        llvm::Value *dataFieldPtr = m_IRBuilder.CreateStructGEP(vecInfo->llvmType, vecAlloca, 0, "data_ptr");
    ...
    if (llvm::AllocaInst *alloca = llvm::dyn_cast<llvm::AllocaInst>(storedValue)) {
      ...
      if (allocatedType->isArrayTy()) {
        ...
        llvm::Value *zero = llvm::ConstantInt::get(llvm::Type::getInt32Ty(m_Context), 0);
        llvm::Value *elemPtr = m_IRBuilder.CreateGEP(allocatedType, alloca, {zero, indexVal}, "arr_idx");
...
void CodeGenerator::VisitIndexAssignExpr(IndexAssignExpr &node) {
  ...
  if (!varName.empty()) {
    ...
    if (!varInfo->isMutable) {
      JLANG_ERROR(
          STR("Cannot assign to element of immutable array '%s' (declared with 'val')", varName.c_str()));
      return;
    }
