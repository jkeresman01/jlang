...
void CodeGenerator::VisitUnaryExpr(UnaryExpr &node) {
  ...
  if (node.op == "!") {
    ...
    if (!operandVal->getType()->isIntegerTy(1)) {
      boolVal =
          m_IRBuilder.CreateICmpNE(operandVal, llvm::ConstantInt::get(operandVal->getType(), 0), "tobool");
    }
    m_LastValue =
        m_IRBuilder.CreateXor(boolVal, llvm::ConstantInt::get(llvm::Type::getInt1Ty(m_Context), 1), "not");
  } else if (node.op == "~") {
...
void CodeGenerator::VisitPrefixExpr(PrefixExpr &node) {
  ...
  if (!varInfo->isMutable) {
    JLANG_ERROR(STR("Cannot modify immutable variable '%s' (declared with 'val')", varExpr->name.c_str()));
    return;
  }
