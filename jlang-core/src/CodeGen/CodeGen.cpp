...
void CodeGenerator::Generate(const std::vector<std::shared_ptr<AstNode>> &program) {
  ...
  for (auto &node : program) {
    if (node && node->type == NodeType::FunctionDecl) {
      ...
      llvm::FunctionType *funcType = llvm::FunctionType::get(MapType(funcNode.returnType), paramTypes, false);
...
void CodeGenerator::DeclareExternalFunctions() {
  ...
  llvm::FunctionType *freeType = llvm::FunctionType::get(llvm::Type::getVoidTy(m_Context), {ptrType}, false);
...
void CodeGenerator::GenerateVtables() {
  for (auto &[structName, siInfo] : m_symbols.GetAllStructInterfaces()) {
    ...
    auto *vtableGlobal =
        new llvm::GlobalVariable(*m_Module, ifaceInfo->vtableType, true, llvm::GlobalValue::InternalLinkage,
