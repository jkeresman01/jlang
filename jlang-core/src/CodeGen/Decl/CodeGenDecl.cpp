...
void CodeGenerator::VisitVariableDecl(VariableDecl &node) {
      ...
      if (srcAlloca->getAllocatedType()->isStructTy()) {
        ...
        if (structTy->hasName()) {
          ...
          if (vecInfo) {
            ...
            m_symbols.DefineVariable(node.name, VariableInfo{srcAlloca, vecTypeRef, false, node.isMutable});
            m_symbols.TrackFunctionLocal(node.name);
            return;
          }
        }
  ...
  if (ifaceInfo && node.initializer) {
    ...
    if (!siInfo || !siInfo->vtableGlobal) {
      JLANG_ERROR(STR("Struct '%s' does not implement interface '%s' (no vtable)", structTypeName.c_str(),
                      node.varType.name.c_str()));
      return;
    }
    ...
    llvm::Value *vtableField = m_IRBuilder.CreateStructGEP(ifaceInfo->fatPtrType, alloca, 1, "fat_vtable");
    ...
    if (node.varType.isArrayType() && node.initializer->type == NodeType::ArrayLiteralExpr) {
      ...
      if (m_LastValue) {
        ...
        if (node.varType.isPointer && !node.varType.isNullable &&
          ...
          JLANG_ERROR(STR("Cannot assign null to non-nullable pointer type '%s*'. Use '%s*?' to allow null.",
