  ...
  return (
    <>
      ...
      <p>
        ...
        He had added it to ALGOL W simply because it was easy to implement. That
        single shortcut went on to infect nearly every language that followed
        &mdash; C, C++, Java, C# &mdash; and has been responsible for countless
        crashes, security vulnerabilities, and late-night debugging sessions
        ever since.
      </p>

      <p>
        Robert C. Martin retells this story in{' '}
        <em>Clean Code: A Handbook of Agile Software Craftsmanship</em>, Chapter
        7 &mdash; "Error Handling", where he lays down two rules:{' '}
        <strong>Don't Return Null</strong> and <strong>Don't Pass Null</strong>.
        His argument is simple &mdash; every <code>null</code> you return is a
        null-check someone will forget to write, and every <code>null</code> you
        pass is a crash waiting to happen.
      </p>
      ...
      <Callout type="important">
        ...
        how this address points to that address, and you&rsquo;re nodding but
        nothing is clicking &mdash; just a wall of fog between you and the idea.
        The second time that happened was Kotlin&rsquo;s <code>
          reified
        </code>{' '}
        keyword and the whole rabbit hole of covariance, contravariance, and
        type erasure vs. reification. Two concepts that made me feel genuinely
        stupid before they finally made me better. That&rsquo;s why they earn
        their place in this language and in this book.
        <br />
        <br />
        And then there&rsquo;s Go. Go proved that pointers don&rsquo;t have to
        be the nightmare that C made them. Go has pointers &mdash; real, honest
        pointers &mdash; but no pointer arithmetic. You can take the address of
        a variable, you can dereference it, but you cannot walk through memory
        like a drunk cowboy with a loaded revolver. Go showed the world that if
        you forbid the dangerous parts and keep the useful parts, pointers
        become a tool instead of a time bomb. That&rsquo;s exactly the bet Jlang
        makes.
        <br />
        <br />
        <iframe
      ...
      <h3>
        1. You cannot assign <code>null</code> to a non-nullable pointer
      </h3>
      <CodeBlock
        code={`var p: Person* = null;  // ERROR: cannot assign null to non-nullable type 'Person*'`}
      />

      <h3>2. Non-nullable pointers must be initialized</h3>
      <CodeBlock
        code={`var p: Person*;  // ERROR: non-nullable pointer 'p' must be initialized`}
      />

      <h3>3. You cannot access members on a nullable pointer directly</h3>
      <CodeBlock
        code={`var q: Person*? = null;
printf("%s", q.Name);  // ERROR: cannot access member 'Name' on nullable type 'Person*?'`}
      ...
      <CodeBlock
        code={`var q: Person*? = findPerson("Alice");
...
printf("Found: %s", p.Name);  // OK: p is non-nullable`}
      ...
      <CodeBlock
        code={`var primary: Config*? = findConfig("main");
      ...
      <table className="doc-table">
        ...
        <tbody>
          <tr>
            ...
            <td>
              Evaluates and returns <code>b</code>
            </td>
          </tr>
        </tbody>
      </table>
