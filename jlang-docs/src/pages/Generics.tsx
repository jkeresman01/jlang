...
fn main() -> i32 {
    ...
    return 0;
      ...
      {/* ── Section 4: How It Works — Monomorphization ── */}
      <h2>How It Works — Monomorphization</h2>
      <p>
        Under the hood, jlang implements generics through a technique called{' '}
        <strong>monomorphization</strong>. When you write{' '}
        <code>max&lt;i32&gt;</code> and <code>max&lt;f64&gt;</code>, the
        compiler doesn't produce a single "generic" function at runtime.
        Instead, it generates two completely separate, type-specialised
        functions — one that operates on <code>i32</code> values and another
        for <code>f64</code> values. The same applies to structs:{' '}
        <code>Pair&lt;i32, f64&gt;</code> becomes a concrete struct type with
        a mangled name like <code>Pair_i32_f64</code>.
      </p>

      <p>
        This means there is <strong>zero runtime overhead</strong> from
        generics — every call is a direct call to a fully-typed function, and
        every struct access uses a known memory layout. The trade-off is
        slightly larger binary sizes when many different type instantiations
        are used.
      </p>

      <Callout type="tip">
        Monomorphization is the same strategy used by C++ templates and Rust
        generics. It trades compile-time work for maximum runtime performance.
        For a great overview of different approaches (type erasure, boxing,
        monomorphization, and more), see{' '}
        <a
