...
void Player::clampPosition() {
    if (y < 0) y = 0;
      ...
      <Callout type="tip">
        ...
        ></iframe>
        <br />
        All jokes aside &mdash; if you look at where Java and C# stand today objectively,
        .NET has pulled ahead of Spring in several areas, and C# as a language has
        become noticeably more elegant and feature-rich than Java. Here are a few
        concrete examples:
        <br /><br />
        <strong>Properties vs. getters/setters:</strong> C# has first-class properties
        built into the language. What takes a field, a getter, and a setter in Java is
        a single line in C#:
        <br /><br />
...
public void setName(String name) { this.name = name; }`}
        ...
        <br />
        <strong>LINQ vs. Streams:</strong> C#&rsquo;s LINQ is integrated into the language
        with query syntax and is far more readable than Java&rsquo;s Stream API, which
        often turns into a wall of chained method calls:
        <br /><br />
        <code style={{ whiteSpace: 'pre', display: 'block', padding: '0.75rem 1rem', background: 'var(--bg-code)', borderRadius: '6px', fontSize: '0.835rem', lineHeight: '1.65' }}>
{`// C# — LINQ query syntax
...
List<Person> adults = people.stream()
    ...
    .collect(Collectors.toList());`}
        ...
        2012. Java only got virtual threads (Project Loom) in 2023, and it still
        lacks the syntactic elegance &mdash; there is no <code>await</code> keyword,
        you just call <code>.join()</code> and hope for the best.
        <br /><br />
        <strong>Pattern matching:</strong> C# supports pattern matching
        with <code>switch</code> expressions, type patterns, property patterns, and
        relational patterns. Java has been catching up since Java 17, but C# has had
        this for years and its syntax is cleaner:
        <br /><br />
        <code style={{ whiteSpace: 'pre', display: 'block', padding: '0.75rem 1rem', background: 'var(--bg-code)', borderRadius: '6px', fontSize: '0.835rem', lineHeight: '1.65' }}>
...
string Classify(object obj) => obj switch
{
    ...
    _                => "unknown"
        ...
        <br />
        <strong>Value types (<code>struct</code>):</strong> C# lets you define value
        types that live on the stack, avoiding heap allocations and GC pressure
        entirely. Java is still working on this with Project Valhalla, which has been
        in development for nearly a decade.
        <br /><br />
        <strong>.NET minimal APIs vs. Spring Boot:</strong> On the framework side,
        .NET&rsquo;s minimal APIs let you spin up a web server in a few lines with
        zero boilerplate. Spring Boot has improved, but it still relies heavily on
        annotations, reflection, and a large dependency tree:
        <br /><br />
        <code style={{ whiteSpace: 'pre', display: 'block', padding: '0.75rem 1rem', background: 'var(--bg-code)', borderRadius: '6px', fontSize: '0.835rem', lineHeight: '1.65' }}>
...
app.Run();`}
        ...
        <br />
        None of this means Java is bad &mdash; it is a proven workhorse that powers
        half the enterprise world. But if you line the two languages up side by side
        today, C# simply has more modern features, less ceremony, and a runtime that
        has been iterating faster. Credit where credit is due.
      </Callout>
    </>
  )
}
